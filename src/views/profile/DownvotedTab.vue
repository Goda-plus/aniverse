...
<script setup>
  ...
  import { getUserDownvotedPosts } from '@/axios/post'
  ...
  // 加载用户已点踩的帖子
  const loadDownvotedPosts = async () => {
    ...
    try {
      loading.value = true
      const response = await getUserDownvotedPosts({
